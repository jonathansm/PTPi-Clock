...
// Approximate character widths for centering
...
int main(int argc, char **argv)
{
    ...
    if (!top_font.LoadFont(TOP_FONT_PATH))
    {
    ...
    if (!bottom_font.LoadFont(BOTTOM_FONT_PATH))
    {
    ...
    while (g_running)
    {
        ...
        if (!g_ptp.have_ptp_time)
        {
            const char *msg = "Wait for PTP";
            int text_width = int(std::strlen(msg)) * TOP_CHAR_WIDTH;
            int x = (width - text_width) / 2;
            if (x < 0)
                x = 0;
            rgb_matrix::DrawText(canvas, top_font, x, top_baseline, color, msg);
        }
        else if (!have_time)
        {
        ...
        else
        {
            ...
            if (get_current_local_time(hh, mm, ss, nsec))
            {
                ...
                int top_width = int(std::strlen(top_line)) * TOP_CHAR_WIDTH;
                int bottom_width = int(std::strlen(bottom_line)) * BOTTOM_CHAR_WIDTH;
